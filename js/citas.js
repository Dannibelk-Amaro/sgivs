$(document).ready(function() {
    let tablaCitasRegistradas = $('#tablaCitasRegistradas').DataTable({
        ...
        "columns": [
            ...
            {"data": "cedula"}, // Cédula Cliente (índice 1)
            {"data": "cedula_representante"}, // Cédula Representante (índice 2)
            {"data": "telefono"}, // Teléfono (índice 3)
            {"data": "id_especialista"}, // Doctor (índice 5)
                    ...
                    respuesta.forEach(function(cita) {
                        ...
                        let botonesAccion = `
                            <button class="btn btn-primary btn-sm m-1 btnModificarCita" data-id="${cita.id}"
                                data-cedula-cliente="${cita.cedula || ''}"
                                data-cedula-representante="${cita.cedula_representante || ''}"
                                data-nombre-cliente="${cita.nombre}"
                                data-apellido-cliente="${cita.apellido}"
                                data-telefono-cliente="${cita.telefono || ''}"
                                data-doctor-atendera="${cita.id_especialista}"
                        ...
                        tablaCitasRegistradas.row.add({
                            cliente_completo: cita.nombre + ' ' + cita.apellido,
                            cedula: cita.cedula || 'N/A',
                            cedula_representante: cita.cedula_representante || 'N/A',
                            telefono: cita.telefono || 'N/A',
    ...
    $('#btnGuardarCita').click(function() {
        ...
        if (!$('#nombreCliente').val() || !$('#apellidoCliente').val() ||
